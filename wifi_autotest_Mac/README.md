...
> Test PC 支援：**macOS** 與 **Ubuntu/Debian Linux**（自動偵測）

---

## 這份專案是什麼？

一個**可以直接跑起來的**自動化測試骨架 —— 不是空殼。你拿到設備、按照 `docs/SETUP.md` 做 15–30 分鐘的實體接線，剩下的軟體環境、測試案例、log 分析都由腳本自動處理。
...
│
├── config/
│   └── config.yaml.example      ← 設定檔範本（cp config/config.yaml.example config.yaml）
│
├── lib/
...
- 設固定 IP：Test PC `192.168.99.10`，DUT `192.168.99.1`，BPI `192.168.99.100`
  - **macOS 用戶**：`bash scripts/macos_network_setup.sh` 互動式設定
  - **Linux 用戶**：netplan 或 NetworkManager
- `ping` 三台都通、SSH 都進得去
...
```bash
bash bootstrap_testpc.sh      # 自動偵測 macOS 或 Ubuntu，安裝對應的工具鏈
source .venv/bin/activate
bash bootstrap_bpi.sh         # 透過 SSH 設定 BPI-R4
```

> macOS 第一次跑會引導你裝 Xcode CLT 跟 Homebrew，整個過程約 10–15 分鐘。
> 之後再跑（更新或重裝）就只剩幾秒。
...
> 檔名刻意加 `z_` 前綴讓 pytest 最後才執行，避免 MT7996 driver 狀態損壞影響其他 6G 測試。詳見 [`Note.md`](docs/Note.md) #5。
...
- **沒有 PDU 整合** —— `config/config.yaml.example` 有註解位置，依你家 PDU API 填
